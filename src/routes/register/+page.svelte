<script lang="ts">
	...
	function getPasswordStrength(pw: string): { label: string; color: string; width: string } {
		...
		if (pw.length < 6) return { label: m.register_password_weak(), color: 'bg-red-500', width: '25%' };
		if (pw.length < 8) return { label: m.register_password_fair(), color: 'bg-yellow-500', width: '50%' };
		if (pw.length < 12 && /[A-Z]/.test(pw) && /[0-9]/.test(pw))
			return { label: m.register_password_strong(), color: 'bg-green-500', width: '75%' };
		if (pw.length >= 12 && /[A-Z]/.test(pw) && /[0-9]/.test(pw) && /[^a-zA-Z0-9]/.test(pw))
			return { label: m.register_password_very_strong(), color: 'bg-emerald-500', width: '100%' };
		return { label: m.register_password_good(), color: 'bg-blue-500', width: '60%' };
	}
...
<div class="flex min-h-[85vh] items-center justify-center px-4 py-16">
	<div class="w-full max-w-md">
		<div class="mb-8 text-center">
			<h1 class="text-3xl font-bold text-white">{m.register_title()}</h1>
			<p class="mt-2 text-sm text-gray-400">{m.register_subtitle()}</p>
		</div>
		...
		<div class="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm">
			...
			>
					...
					{#if showPasswordStrength && password.length > 0}
						<div class="mt-2">
							...
							<p class="mt-1 text-xs text-gray-400">
								Strength: <span class="font-medium text-white">{strength.label}</span>
							</p>
						</div>
					{/if}
				</div>
				...
				>
					{loading ? m.register_loading() : m.register_submit()}
				</button>
			</form>
			...
			<div class="relative my-6">
				...
				<div class="relative flex justify-center text-sm">
					<span class="bg-transparent px-2 text-gray-500">{m.register_divider()}</span>
				</div>
			</div>
